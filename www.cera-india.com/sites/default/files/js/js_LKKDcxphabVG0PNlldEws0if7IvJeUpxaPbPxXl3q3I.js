...
(function($, drupalSettings) {
    ...
    Drupal.behaviors.webformClientSideValidationAjax = {
        attach: function(context) {
            $('form.webform-submission-form .form-actions :submit:not([formnovalidate])').once('webform-clientside-validation-ajax').addClass('cv-validate-before-ajax');
        }
    };
...
(function($, Drupal) {
    $(document).ready(function() {
        $('.map-container ul li:first-child').addClass('active');
        $('#myCarousel .item:first-child').addClass('active');
        $('#edit-keys, #edit-submit').removeClass('form-submit form-control');
        $('#edit-keys--2, #edit-submit--3').removeClass('form-submit form-control');
        $('.privacy-policy-wrapper article > div').removeClass('content');
    ...
    $('.select-park').on('change', function() {
        var items = $(this).children("option:selected").val();
        if (items) {
            $('input[name=link]').attr('value', items);
        } else {
            $('input[name=link]').removeAttr('value', items);
        }
    });
    $('.form-item-i-agree-to-receive-emailers-newsletters-and-other-communication-').wrap("<div class='row'></div>");
